...
import { 
  ...
  History, ArrowLeft, Plus, Edit3, Download, Check, Package, 
  Upload, Image as ImageIcon, LayoutDashboard, Search, Calendar, DollarSign 
} from "lucide-react";

// --- TYPES (Fixes the Build Error) ---
interface Product {
  id: number;
  name: string;
  hsn: string;
  unit: string;
  rate: number;
  image: string | null; // This line fixes the Vercel error
}
...
export default function App() {
  ...
  // Data State (Typed correctly now)
  ...
  const [newProduct, setNewProduct] = useState<{name: string, hsn: string, unit: string, rate: string, image: string | null}>({ name: "", hsn: "", unit: "Kgs", rate: "", image: null });
  ...
  const handleAddCustomer = () => {
    ...
    const c: Customer = { id: Date.now(), ...newCustomer };
          ...
          <div className="w-full bg-white rounded shadow-lg p-8 overflow-y-auto no-print">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
               <div className="bg-blue-50 p-6 rounded border border-blue-200 flex items-center justify-between">
                  <div><p className="text-xs font-bold text-blue-500 uppercase">Total Revenue</p><p className="text-2xl font-bold text-blue-900">₹ {getTotalRevenue().toLocaleString()}</p></div>
               ...
               <div className="bg-red-50 p-6 rounded border border-red-200 flex items-center justify-between">
                  <div><p className="text-xs font-bold text-red-500 uppercase">Pending Payments</p><p className="text-2xl font-bold text-red-900">₹ {getPendingAmount().toLocaleString()}</p></div>
               ...
               <div className="bg-yellow-50 p-6 rounded border border-yellow-200 flex items-center justify-between">
                  <div><p className="text-xs font-bold text-yellow-600 uppercase">Total Invoices</p><p className="text-2xl font-bold text-yellow-900">{savedInvoices.length}</p></div>
            ...
            <div className="bg-gray-50 p-4 rounded border mb-6 flex flex-wrap gap-4 items-end">
               <div className="flex-1 min-w-[200px]">
                  ...
                  <div className="flex items-center bg-white border rounded p-2 mt-1">
                     ...
                     <input placeholder="e.g. 20/25-26 or ABC..." className="w-full outline-none text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                  </div>
               </div>
               <div><label className="text-xs font-bold text-gray-500">From Date</label><input type="date" className="border p-2 rounded mt-1 block text-sm bg-white" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} /></div>
               <div><label className="text-xs font-bold text-gray-500">To Date</label><input type="date" className="border p-2 rounded mt-1 block text-sm bg-white" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} /></div>
            ...
            <div className="border rounded overflow-hidden">
               <table className="w-full text-sm">
                  <thead className="bg-[#0B2447] text-white">
                     <tr><th className="p-3 text-left">Date</th><th className="p-3 text-left">Inv No</th><th className="p-3 text-left">Customer</th><th className="p-3 text-right">Amount</th><th className="p-3 text-center">Status</th><th className="p-3 text-center">Action</th></tr>
                  </thead>
                  <tbody>
                     {getFilteredInvoices().length === 0 ? (
                        ...
                        getFilteredInvoices().map(inv => (
                           <tr key={inv.id} className="border-b hover:bg-gray-50">
                              <td className="p-3">{inv.date}</td><td className="p-3 font-bold">{inv.invNo}</td><td className="p-3">{inv.customerName}</td>
                              <td className="p-3 text-right font-bold">₹{inv.amount.toLocaleString()}</td>
                              <td className="p-3 text-center">
                                 <button onClick={() => togglePaymentStatus(inv.id)} className={`px-3 py-1 rounded-full text-[10px] font-bold ${inv.status === "PAID" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{inv.status}</button>
                              </td>
                              <td className="p-3 text-center">
                                 <button onClick={() => loadAndPrintOldInvoice(inv)} className="text-blue-600 hover:underline font-bold text-xs flex items-center justify-center gap-1"><Printer size={14}/> View/Print</button>
                              </td>
                           </tr>
                        ))
                     )}
